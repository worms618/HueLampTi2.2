...
namespace HueLampApp
{
    ...
    {
        public static Color getColor(Tuple<int,long,int> hsv)
        {
            double hue = ((double)hsv.Item2 * 360.0f) / 65535.0f;
            double sat = (double)hsv.Item3 / 255.0f;
            double val = (double)hsv.Item1 / 255.0f;
        ...
        public static Color getColor(this HueLamp light)
        {
            ...
            //double sat = (double)light.Sat / 255.0f;
            //double val = (double)light.Brightness / 255.0f;

            //int r, g, b;
            //HsvToRgb(hue, sat, val, out r, out g, out b);
            //return Color.FromArgb(255, Convert.ToByte(r), Convert.ToByte(g), Convert.ToByte(b));
            return getColor(light.HSV);
        }
